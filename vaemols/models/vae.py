...
class EncoderConv1D(tf.keras.Model):
    ...
    def call(self, inputs):
        with v1.variable_scope(self.name, reuse=v1.AUTO_REUSE):
            x = self.conv1(inputs)
            x = self.norm1(x)
            x = self.act1(x)
            ...
            self.dist = v1.distributions.Normal(loc=z_mean, scale=tf.exp(0.5*z_log_var)) # z_log_var is actually the logarithm of sigma^2 = 2*log(sigma)
...
class VariationalAutoencoder(tf.keras.Model):
    ...
    def vae_loss_func(self, y_true, y_pred):
        ...
        recon_loss = v1.losses.sparse_softmax_cross_entropy(y_true_rep, self.outputs_logits, reduction=v1.losses.Reduction.SUM)
