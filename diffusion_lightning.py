...
class DDP(pl.LightningModule):
    ...
    def training_step(self, batch, batch_nb):
        ...
        img, _ = batch
        time   = (torch.rand(img.shape[0]) * self.conf.model.schedule.n_timestep).type(torch.int64).to(img.device)
    ...
    def validation_step(self, batch, batch_nb):
        """Use self.ema to compute the diffusion training loss

        Args:
            batch (_type_): batch
            batch_nb (_type_): _description_
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_gpu", type=int, default=0, help="Number of available GPUs.")
